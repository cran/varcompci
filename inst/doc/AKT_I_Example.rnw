...
It is assumed that $A_{gi}$ is a fixed effect whileas $B_{gj(i)}$ and $e_{gijk}$큦 are normal distributed with 0 expected value and $\sigma_B$, $\sigma_e$ respectively, and that these two sequences of random variables are independent of each other. The data were evaluated by analysis of variance (ANOVA) based on this nested model.
...
It is assumed that $A_{gi}$ is a fixed effect whileas $B_{gj(i)}$ and $e_{gijk}$큦 are normal distributed with 0 expected value and $\sigma_B$, $\sigma_e$ respectively, and that these two sequences of random variables are independent of each other. 
...
\bibitem{Krech T et al 2010} Krech T, Thiede M, Hilgenberg E, Sch채fer R, J체rchott K. Characterization of
